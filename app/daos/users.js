...
exports.getAllUser = (next) => {
	var strSQL = mysql.format('SELECT U.*, MD5(U.id) as _id FROM users U;');
...
exports.checkUsernameDuplicate2 = (_id, username, next) => {
	var strSQL = mysql.format('SELECT * FROM users WHERE username = ? AND MD5(id) <> ?', [username, _id]);
	db.query(strSQL, next);
}

exports.getUser = (user_id, next) => {
	var strSQL = mysql.format('SELECT *, MD5(id) as _id  FROM users WHERE MD5(id)=? LIMIT 1', [user_id]);
	db.query(strSQL, next);	
};

exports.deleteUser = (user_id, next) => {
	var strSQL = mysql.format('DELETE FROM users WHERE MD5(id)=?', [user_id]);
	db.actionQuery(strSQL, next);	
}

exports.updateUser = (user_id, user, next) => {
	var strSQL = mysql.format('UPDATE users SET firstname=?, lastname=?, username=?, password=? WHERE MD5(id)=? LIMIT 1', [
